...
# ==================== 修改1：MultiTaskHead（与LTC结构对齐）====================
class MultiTaskHead(nn.Module):
    def __init__(self, input_dim=256, dropout_rate=0.22):  # 修改：input_dim=256适配GRU
        super(MultiTaskHead, self).__init__()
        print(f"[INFO] MultiTaskHead initialized with input_dim={input_dim} (GRU Baseline)")

        # 修改2：网络结构与LTC一致
        self.shared = nn.Sequential(
            nn.Conv2d(input_dim, 384, kernel_size=1),  # 修改：384而非512
            nn.BatchNorm2d(384),
            nn.ReLU(),
...
# ==================== 修改3：GRU模型（与LTC架构对齐）====================
class GRUWindSpeedPredictor(nn.Module):
    """
    GRU风速预测模型 - 与LTC架构对齐版本
    修改要点：
    1. 删除空间CNN分支，只保留时序分支（与LTC一致）
    2. 添加output_layer（与LTC一致）
    3. 数据流与LTC完全一致
    """

    def __init__(self, H, W, tigge_features=10, dropout_rate=0.22,  # 修改4：GRU适配的dropout
                 gru_hidden_dim=256, gru_layers=2):  # 修改5：hidden_dim=256
        super(GRUWindSpeedPredictor, self).__init__()
        print("[INFO] GRUWindSpeedPredictor initialized - TEMPORAL-ONLY (Aligned with LTC architecture)")
        ...
        self.gru = nn.GRU(
            input_size=tigge_features + 5,  # 10 + 5 = 15
        ...
        # ==================== 修改8：删除gru_to_spatial和fusion层 ====================
        # 原代码：self.gru_to_spatial = nn.Sequential(...)  # 已删除
        # 原代码：self.fusion = nn.Sequential(...)  # 已删除

        # ==================== MultiTaskHead ====================
        self.multi_task_head = MultiTaskHead(
            input_dim=gru_hidden_dim,  # 修改9：直接使用GRU的hidden_dim
        ...
        print(f"[INFO] GRU Model Total parameters: {total_params:,}")
        print(f"[INFO] GRU Model Trainable parameters: {trainable_params:,}")
    ...
    def forward(self, tigge_spatial, dem_spatial, interaction_spatial,
        ...
        gru_input = torch.cat([tigge_seq_reshaped, time_seq_expanded], dim=-1)

        # ==================== GRU处理时序 ====================
        gru_out, h_n = self.gru(gru_input)  # gru_out: (B*H*W, seq_len, hidden_dim)

        # 取最后一个时间步的隐藏状态（与LTC一致）
        h = gru_out[:, -1, :]  # (B*H*W, hidden_dim)

        # ==================== 修改12：通过output_layer（与LTC一致） ====================
...
class MultiTaskLoss(nn.Module):
    def __init__(self, alpha=1.5, beta=0.26, gamma=0.28):  # 修改：GRU适配的权重
        super(MultiTaskLoss, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.point_criterion = nn.SmoothL1Loss()
        print(f"[INFO] MultiTaskLoss权重 (GRU Baseline): α={alpha}, β={beta}, γ={gamma}")
...
# ==================== 可视化函数（保持不变） ====================
def visualize_predictions(model, dataloader, device, save_dir='visualizations_GRU_task2', num_samples=5):
...
def train_model(model, train_loader, val_loader, device,
                epochs=10, learning_rate=0.00035, weight_decay=0.00004,  # 修改：GRU适配
                patience=3, accumulation_steps=4):
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Started training process (GRU Baseline - Aligned with LTC)")

    criterion = MultiTaskLoss(alpha=1.5, beta=0.26, gamma=0.28)  # GRU适配
    ...
    patience_counter = 0
    os.makedirs('checkpoints_GRU_task2', exist_ok=True)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Checkpoint directory created: checkpoints_GRU_task2")
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'checkpoints_GRU_task2/best_model_GRU_task2.pth')
        ...
        torch.save({
            ...
            'val_mc_95': val_mc_95,
            'val_mc_50': val_mc_50
        }, f'checkpoints_GRU_task2/checkpoint_epoch_{epoch + 1}_GRU_task2.pth')
    ...
    plt.title('GRU Baseline (Aligned) - Training and Validation Loss', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('loss_curve_GRU_task2.png', dpi=300)
    plt.close()
    ...
    plt.title('GRU Baseline (Aligned) - Mean Absolute Error', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('mae_curve_GRU_task2.png', dpi=300)
    plt.close()
    ...
    plt.title('GRU Baseline (Aligned) - Root Mean Squared Error', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('rmse_curve_GRU_task2.png', dpi=300)
    plt.close()
    ...
    plt.title('GRU Baseline (Aligned) - R² Score', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('r2_curve_GRU_task2.png', dpi=300)
    plt.close()
    ...
    plt.title('GRU Baseline (Aligned) - Continuous Ranked Probability Score', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('crps_curve_GRU_task2.png', dpi=300)
    plt.close()
    ...
    axes[0].set_title('GRU Baseline (Aligned) - 95% CI Coverage', fontsize=14)
    axes[0].legend()
    axes[0].grid(True)
    ...
    axes[1].set_title('GRU Baseline (Aligned) - 50% CI Coverage', fontsize=14)
    axes[1].legend()
    axes[1].grid(True)

    plt.tight_layout()
    plt.savefig('coverage_curve_GRU_task2.png', dpi=300)
    plt.close()
    ...
    axes[0].set_title('GRU Baseline (Aligned) - 95% Interval Width', fontsize=14)
    axes[0].legend()
    axes[0].grid(True)
    ...
    axes[1].set_title('GRU Baseline (Aligned) - 50% Interval Width', fontsize=14)
    axes[1].legend()
    axes[1].grid(True)

    plt.tight_layout()
    plt.savefig('width_curve_GRU_task2.png', dpi=300)
    plt.close()
    ...
    axes[0].set_title('GRU Baseline (Aligned) - 95% Interval Efficiency', fontsize=14)
    axes[0].legend()
    axes[0].grid(True)
    ...
    axes[1].set_title('GRU Baseline (Aligned) - 50% Interval Efficiency', fontsize=14)
    axes[1].legend()
    axes[1].grid(True)

    plt.tight_layout()
    plt.savefig('mc_curve_GRU_task2.png', dpi=300)
    plt.close()
    ...
    visualize_predictions(model, val_loader, device, save_dir='visualizations_GRU_task2', num_samples=5)
...
if __name__ == "__main__":
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Program started (GRU Baseline - Aligned with LTC)")
    ...
    batch_size = 8
    epochs = 10
    learning_rate = 0.00035  # 修改15：GRU适配的学习率
    weight_decay = 0.00004  # 修改16：GRU适配的权重衰减
    patience = 3
    ...
    # 修改17：初始化对齐后的GRU模型
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Initializing GRU model (Aligned with LTC)")
    model = GRUWindSpeedPredictor(
        H, W,
        tigge_features=10,
        dropout_rate=0.22,  # 修改18：GRU适配的dropout（比LTC稍低，GRU收敛快）
        gru_hidden_dim=256,  # 修改19：GRU隐藏维度（GRU参数效率高，256足够）
        gru_layers=2  # 保持2层
    ).to(device)
