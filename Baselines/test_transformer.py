...
class MultiTaskHead(nn.Module):
    def __init__(self, input_dim=256, dropout_rate=0.15):
...
class TransformerWindSpeedPredictor(nn.Module):
    ...
    def forward(self, tigge_spatial, dem_spatial, interaction_spatial,
        ...
        h = torch.sum(x * attn_weights, dim=1)
...
def test_model(model, test_loader, device, save_dir):
    ...
    np.save(os.path.join(save_dir, 'Transformer_test_point_preds.npy'), all_point_preds_orig)
    np.save(os.path.join(save_dir, 'Transformer_test_targets.npy'), all_targets_orig)
    np.save(os.path.join(save_dir, 'Transformer_test_dates.npy'), all_dates)
    np.save(os.path.join(save_dir, 'Transformer_test_mus.npy'), all_mus_orig)
    np.save(os.path.join(save_dir, 'Transformer_test_sigmas.npy'), all_sigmas_orig)
    np.save(os.path.join(save_dir, 'Transformer_test_intervals.npy'), all_interval_preds_orig)
    ...
    with open(os.path.join(save_dir, 'Transformer_yearly_metrics_point.json'), 'w') as f:
        json.dump(yearly_metrics['point'], f, indent=2)
    with open(os.path.join(save_dir, 'Transformer_yearly_metrics_interval.json'), 'w') as f:
        json.dump(yearly_metrics['interval'], f, indent=2)
    with open(os.path.join(save_dir, 'Transformer_yearly_metrics_probabilistic.json'), 'w') as f:
    ...
    with open(os.path.join(save_dir, 'Transformer_seasonal_metrics_point.json'), 'w') as f:
        json.dump(seasonal_metrics_point, f, indent=2)
    with open(os.path.join(save_dir, 'Transformer_seasonal_metrics_interval.json'), 'w') as f:
        json.dump(seasonal_metrics_interval, f, indent=2)
    with open(os.path.join(save_dir, 'Transformer_seasonal_metrics_probabilistic.json'), 'w') as f:
    ...
    with open(os.path.join(save_dir, 'Transformer_monthly_metrics_point.json'), 'w') as f:
        json.dump(monthly_metrics_point, f, indent=2)
    with open(os.path.join(save_dir, 'Transformer_monthly_metrics_interval.json'), 'w') as f:
        json.dump(monthly_metrics_interval, f, indent=2)
    with open(os.path.join(save_dir, 'Transformer_monthly_metrics_probabilistic.json'), 'w') as f:
    ...
    with open(os.path.join(save_dir, 'Transformer_hourly_metrics_point.json'), 'w') as f:
        json.dump(hourly_metrics_point, f, indent=2)
    with open(os.path.join(save_dir, 'Transformer_hourly_metrics_interval.json'), 'w') as f:
        json.dump(hourly_metrics_interval, f, indent=2)
    with open(os.path.join(save_dir, 'Transformer_hourly_metrics_probabilistic.json'), 'w') as f:
        json.dump(hourly_metrics_probabilistic, f, indent=2)

    # ========== 5. 保存区间可视化数据 ==========
    print(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] Saving interval predictions for visualization...")
    np.savez(
        os.path.join(save_dir, 'Transformer_interval_visualization_data.npz'),
        dates=all_dates,
    ...
    # 最终汇总
    print(f"\n\n{'#' * 80}")
    print(f"{'FINAL SUMMARY - TRANSFORMER MODEL':^80}")
    print(f"{'#' * 80}")
    print(f"\n[Files Saved to: {save_dir}]")
    print(f"\nSaved Files:")
    print(f"  - Transformer_yearly_metrics_point.json")
    print(f"  - Transformer_yearly_metrics_interval.json")
    print(f"  - Transformer_yearly_metrics_probabilistic.json")
    print(f"  - Transformer_seasonal_metrics_*.json")
    print(f"  - Transformer_monthly_metrics_*.json")
    print(f"  - Transformer_hourly_metrics_*.json")
    print(f"  - Transformer_test_*.npy")
    print(f"  - Transformer_interval_visualization_data.npz")
...
if __name__ == "__main__":
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] ===== TRANSFORMER MODEL TESTING =====")
    ...
    # 加载模型权重
    model_path = r"E:\yym2\qixiang\Obs_PDF\src\checkpoints_transformer_task2\best_model_transformer_task2.pth"
    ...
    # 设置保存目录
    save_dir = r"E:\yym2\qixiang\Obs_PDF\src\resluts_task2\Transformer"
