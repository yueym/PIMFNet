...
class MultiTaskHead(nn.Module):
    def __init__(self, input_dim=128, dropout_rate=0.1):
        ...
        print(f"[INFO] MultiTaskHead initialized with input_dim={input_dim} (DeepAR)")
...
class GaussianOutput(nn.Module):
    ...
    def __init__(self, input_dim, output_dim=1):
        ...
        nn.init.xavier_uniform_(self.sigma_layer.weight, gain=0.1)
        nn.init.constant_(self.mu_layer.bias, 0.5)
        nn.init.constant_(self.sigma_layer.bias, -2.0)
...
class DeepAREncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # 使用最后时刻的输出
        context = lstm_out[:, -1, :]  # (batch, hidden_dim)
...
class DeepARWindSpeedPredictor(nn.Module):
    ...
    def __init__(self, H, W, tigge_features=10, dropout_rate=0.1,
        ...
        print("[INFO] DeepARWindSpeedPredictor initialized - Multi-Task Version")
        ...
        print(f"[INFO] DeepAR Model Configuration:")
...
def gaussian_nll_loss(mu, sigma, target):
    sigma = torch.clamp(sigma, min=0.01, max=0.3)
    nll = 0.5 * torch.log(2 * np.pi * sigma ** 2) + 0.5 * ((target - mu) ** 2) / (sigma ** 2)
    nll_mean = nll.mean()

    if torch.isnan(nll_mean) or torch.isinf(nll_mean):
        return torch.tensor(0.0, device=mu.device, dtype=mu.dtype)

    return torch.clamp(nll_mean, max=5.0)
...
# ==================== DeepAR多任务损失 ====================
class DeepARMultiTaskLoss(nn.Module):
    def __init__(self, alpha=1.2, beta=0.25, gamma=0.25, nll_weight=0.3):
        super(DeepARMultiTaskLoss, self).__init__()
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.nll_weight = nll_weight
        self.point_criterion = nn.SmoothL1Loss()
        print(f"[INFO] DeepARMultiTaskLoss: α={alpha}, β={beta}, γ={gamma}, nll_weight={nll_weight}")
    ...
    def forward(self, point_pred, mu, sigma, interval_pred, target):
        ...
        # 高斯NLL损失
        loss_nll = gaussian_nll_loss(mu, sigma, target)
...
# ==================== 可视化函数 ====================
def visualize_predictions(model, dataloader, device, save_dir='visualizations_DeepAR_task2', num_samples=5):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dataloader):
            ...
            axes[0].set_title(f'DeepAR Probabilistic Prediction (Sample {batch_idx + 1})', fontsize=14)
            axes[0].legend()
            ...
            axes[1].set_title(f'DeepAR Interval Prediction (Sample {batch_idx + 1})', fontsize=14)
            axes[1].legend()
...
def train_model(model, train_loader, val_loader, device,
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Started training process (DeepAR - Multi-Task)")

    criterion = DeepARMultiTaskLoss(alpha=1.2, beta=0.25, gamma=0.25, nll_weight=0.3)
    ...
    patience_counter = 0
    os.makedirs('checkpoints_DeepAR_task2', exist_ok=True)
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Checkpoint directory created: checkpoints_DeepAR_task2")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            try:
                ...
                epoch_nll_sum += loss_dict['nll_loss']
                ...
                if batch_idx % 50 == 0 or batch_idx == len(train_loader) - 1:
                    ...
                    print(
                        ...
                        f"NLL:{loss_dict['nll_loss']:.4f}, C:{loss_dict['crps_loss']:.4f}, "
                        f"I:{loss_dict['interval_loss']:.4f}], Time: {batch_time:.2f}s")
        ...
        train_nll.append(epoch_nll_sum / batch_count if batch_count > 0 else 0)
        ...
        print(f'  CRPS: {train_crps_val:.4f} | NLL: {train_nll[-1]:.4f}')
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                try:
                    ...
                    val_nll_sum += loss_dict['nll_loss']
        ...
        val_nll.append(val_nll_sum / val_batch_count if val_batch_count > 0 else 0)
        ...
        print(f'  CRPS: {val_crps_val:.4f} | NLL: {val_nll[-1]:.4f}')
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'checkpoints_DeepAR_task2/best_model_DeepAR_task2.pth')
        ...
        }, f'checkpoints_DeepAR_task2/checkpoint_epoch_{epoch + 1}_DeepAR_task2.pth')
    ...
    plt.title('DeepAR - Training and Validation Loss', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('loss_curve_DeepAR_task2.png', dpi=300)
    plt.close()
    ...
    plt.title('DeepAR - Mean Absolute Error', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('mae_curve_DeepAR_task2.png', dpi=300)
    plt.close()
    ...
    plt.title('DeepAR - Root Mean Squared Error', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('rmse_curve_DeepAR_task2.png', dpi=300)
    plt.close()
    ...
    plt.title('DeepAR - R² Score', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('r2_curve_DeepAR_task2.png', dpi=300)
    plt.close()
    ...
    plt.title('DeepAR - Continuous Ranked Probability Score', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('crps_curve_DeepAR_task2.png', dpi=300)
    plt.close()

    # NLL曲线
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, len(train_nll) + 1), train_nll, label='Train NLL', color='blue', linewidth=2)
    plt.plot(range(1, len(val_nll) + 1), val_nll, label='Validation NLL', color='orange', linewidth=2)
    plt.xlabel('Epoch', fontsize=12)
    plt.ylabel('Negative Log-Likelihood', fontsize=12)
    plt.title('DeepAR - Gaussian NLL', fontsize=14)
    plt.legend()
    plt.grid(True)
    plt.savefig('nll_curve_DeepAR_task2.png', dpi=300)
    plt.close()
    ...
    axes[0].set_title('DeepAR - 95% CI Coverage', fontsize=14)
    axes[0].legend()
    axes[0].grid(True)
    ...
    axes[1].set_title('DeepAR - 50% CI Coverage', fontsize=14)
    axes[1].legend()
    axes[1].grid(True)
    plt.tight_layout()
    plt.savefig('coverage_curve_DeepAR_task2.png', dpi=300)
    plt.close()
    ...
    axes[0].set_title('DeepAR - 95% Interval Width', fontsize=14)
    axes[0].legend()
    axes[0].grid(True)
    ...
    axes[1].set_title('DeepAR - 50% Interval Width', fontsize=14)
    axes[1].legend()
    axes[1].grid(True)
    plt.tight_layout()
    plt.savefig('width_curve_DeepAR_task2.png', dpi=300)
    plt.close()
    ...
    axes[0].set_title('DeepAR - 95% Interval Efficiency', fontsize=14)
    axes[0].legend()
    axes[0].grid(True)
    ...
    axes[1].set_title('DeepAR - 50% Interval Efficiency', fontsize=14)
    axes[1].legend()
    axes[1].grid(True)
    plt.tight_layout()
    plt.savefig('mc_curve_DeepAR_task2.png', dpi=300)
    plt.close()
    ...
    visualize_predictions(model, val_loader, device, save_dir='visualizations_DeepAR_task2', num_samples=5)
...
if __name__ == "__main__":
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Program started (DeepAR - Multi-Task)")
    print("=" * 80)
    ...
    # 初始化DeepAR模型
    print(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] Initializing DeepAR model")
