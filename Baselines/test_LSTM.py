...
class MultiTaskHead(nn.Module):
    def __init__(self, input_dim=216, dropout_rate=0.2):
        ...
        # 修正：这里第二个卷积层的输入应该是 128，不是 256
        self.prob_log_sigma = nn.Sequential(
            nn.Conv2d(256, 128, kernel_size=1),  # 第一层：256->128
            nn.ReLU(),
            nn.Dropout(p=dropout_rate),
            nn.Conv2d(128, 1, kernel_size=1)  # 第二层：128->1（这里之前错写成了256->1）
        )
...
class LSTMWindSpeedPredictor(nn.Module):
    ...
    def __init__(self, H, W, tigge_features=10, dropout_rate=0.2,
                 lstm_hidden_dim=216, lstm_layers=2):
...
def test_model(model, test_loader, device, save_dir):
    ...
    np.save(os.path.join(save_dir, 'LSTM_test_point_preds.npy'), all_point_preds_orig)
    np.save(os.path.join(save_dir, 'LSTM_test_targets.npy'), all_targets_orig)
    np.save(os.path.join(save_dir, 'LSTM_test_dates.npy'), all_dates)
    np.save(os.path.join(save_dir, 'LSTM_test_mus.npy'), all_mus_orig)
    np.save(os.path.join(save_dir, 'LSTM_test_sigmas.npy'), all_sigmas_orig)
    np.save(os.path.join(save_dir, 'LSTM_test_intervals.npy'), all_interval_preds_orig)
    ...
    with open(os.path.join(save_dir, 'LSTM_yearly_metrics_point.json'), 'w') as f:
        json.dump(yearly_metrics['point'], f, indent=2)
    with open(os.path.join(save_dir, 'LSTM_yearly_metrics_interval.json'), 'w') as f:
        json.dump(yearly_metrics['interval'], f, indent=2)
    with open(os.path.join(save_dir, 'LSTM_yearly_metrics_probabilistic.json'), 'w') as f:
    ...
    with open(os.path.join(save_dir, 'LSTM_seasonal_metrics_point.json'), 'w') as f:
        json.dump(seasonal_metrics_point, f, indent=2)
    with open(os.path.join(save_dir, 'LSTM_seasonal_metrics_interval.json'), 'w') as f:
        json.dump(seasonal_metrics_interval, f, indent=2)
    with open(os.path.join(save_dir, 'LSTM_seasonal_metrics_probabilistic.json'), 'w') as f:
    ...
    with open(os.path.join(save_dir, 'LSTM_monthly_metrics_point.json'), 'w') as f:
        json.dump(monthly_metrics_point, f, indent=2)
    with open(os.path.join(save_dir, 'LSTM_monthly_metrics_interval.json'), 'w') as f:
        json.dump(monthly_metrics_interval, f, indent=2)
    with open(os.path.join(save_dir, 'LSTM_monthly_metrics_probabilistic.json'), 'w') as f:
    ...
    with open(os.path.join(save_dir, 'LSTM_hourly_metrics_point.json'), 'w') as f:
        json.dump(hourly_metrics_point, f, indent=2)
    with open(os.path.join(save_dir, 'LSTM_hourly_metrics_interval.json'), 'w') as f:
        json.dump(hourly_metrics_interval, f, indent=2)
    with open(os.path.join(save_dir, 'LSTM_hourly_metrics_probabilistic.json'), 'w') as f:
        json.dump(hourly_metrics_probabilistic, f, indent=2)

    # ========== 5. 保存区间可视化数据 ==========
    print(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] Saving interval predictions for visualization...")
    np.savez(
        os.path.join(save_dir, 'LSTM_interval_visualization_data.npz'),
        dates=all_dates,
    ...
    # 最终汇总
    print(f"\n\n{'#' * 80}")
    print(f"{'FINAL SUMMARY - LSTM MODEL':^80}")
    print(f"{'#' * 80}")
    print(f"\n[Files Saved to: {save_dir}]")
    print(f"\nSaved Files:")
    print(f"  - LSTM_yearly_metrics_point.json")
    print(f"  - LSTM_yearly_metrics_interval.json")
    print(f"  - LSTM_yearly_metrics_probabilistic.json")
    print(f"  - LSTM_seasonal_metrics_*.json")
    print(f"  - LSTM_monthly_metrics_*.json")
    print(f"  - LSTM_hourly_metrics_*.json")
    print(f"  - LSTM_test_*.npy")
    print(f"  - LSTM_interval_visualization_data.npz")
...
if __name__ == "__main__":
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] ===== LSTM MODEL TESTING =====")
    ...
    # 加载模型权重
    model_path = r"E:\yym2\qixiang\Obs_PDF\src\checkpoints_LSTM_task2\best_model_LSTM_task2.pth"
    ...
    # 设置保存目录
    save_dir = r"E:\yym2\qixiang\Obs_PDF\src\resluts_task2\LSTM"
